...
def create_cave(rocks, borders):
	cave = [['.'] * (borders['maxX'] + 1) for i in range(borders['maxY'] + 1)]
	...
	while True:
		...
		while can_drop(cave, sand) == True:
			...
			if cave[sand[1] + 1][sand[0]] == '.':
				...
				sand[1] += 1
				sand[0] += 1
			if sand[1] + 1 >= len(cave) or (sand[1] + 1 and sand[0] + 1 >= len(cave[0])) or (sand[1] + 1 and sand[0] - 1 < 0):
				return dropped
		dropped += 1
...
print(f"The total grains of sand that dropped is {dropped}.")
